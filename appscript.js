...
/**
 * Handles condition changes in column E
 * @param {number} row The row number where condition changed
 */
...
function fetchSkuForRow(row) {
  ...
  // Get the printing from column D, default to "Normal"
  let printingInput = sheet.getRange(row, 4).getValue().toString().trim(); // Column D
  const printingsToFetch = printingInput ? [printingInput] : ['Normal'];
  console.log(`Row ${row}: Using printings: ${printingsToFetch.join(', ')}`);

  // Get the condition from column E, default to "Near Mint"
  let conditionInput = sheet.getRange(row, 5).getValue().toString().trim(); // Column E
  const conditionsToFetch = conditionInput ? [conditionInput] : ['Near Mint'];
...
/**
 * Utility function to clear all data in columns C, F, G, H, I, M for fresh processing
 */
/**
...
function testApiConnection() {
  ...
  try {
    ...
    if (responseCode === 200) {
      const jsonResponse = JSON.parse(responseBody);
      if (jsonResponse.errors) {
        ui.alert('API Test Failed', 'GraphQL errors: ' + JSON.stringify(jsonResponse.errors), ui.ButtonSet.OK);
      } else if (jsonResponse.data && jsonResponse.data.cards) {
        ui.alert('API Test Successful', 'Successfully connected to MTGGraphQL API!', ui.ButtonSet.OK);
      } else {
        ui.alert('API Test Warning', 'Connected but received unexpected response format.', ui.ButtonSet.OK);
      }
    } else {
      ui.alert('API Test Failed', `HTTP ${responseCode}: ${responseBody}`, ui.ButtonSet.OK);
    }
  } catch (e) {
    SpreadsheetApp.getUi().alert('API Test Error', 'Connection failed: ' + e.message, ui.ButtonSet.OK);
  }
}
/**
 * Simple function to trigger authorization for external requests
 * Run this function first to authorize UrlFetchApp permissions
 */
function authorizeExternalRequests() {
  try {
    // Simple test request to trigger authorization
    const response = UrlFetchApp.fetch('https://httpbin.org/get', {
      method: 'GET',
      muteHttpExceptions: true
    });

    SpreadsheetApp.getUi().alert('Success', 'External request permissions authorized!', SpreadsheetApp.getUi().ButtonSet.OK);
  } catch (error) {
    SpreadsheetApp.getUi().alert('Error', 'Authorization failed: ' + error.message, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
/**
 * Debug function to check token storage and authorization status
 */
/**
 * Enhanced debug function to see the actual API error response
 */
function debugApiResponse() {
  const ui = SpreadsheetApp.getUi();
  const storedToken = PropertiesService.getUserProperties().getProperty('MTGGRAPHQL_ACCESS_TOKEN');

  if (!storedToken) {
    ui.alert('Error', 'No API token found', ui.ButtonSet.OK);
    return;
  }
  ...
  try {
    ...
    let debugInfo = [];
    debugInfo.push(`Status Code: ${statusCode}`);
    debugInfo.push(`Response: ${responseText.substring(0, 500)}${responseText.length > 500 ? '...' : ''}`);

    ui.alert('API Debug Response', debugInfo.join('\n\n'), ui.ButtonSet.OK);

  } catch (error) {
    console.error('Request failed:', error);
    ui.alert('Request Error', error.message, ui.ButtonSet.OK);
  }
}
...
MTGGRAPHQL_ENDPOINT = "https://api.tcgplayer.com/pricing/marketprices/graphql"

def fetch_sku_by_uuid(uuid, conditions, printings):
  query = f"""
  query {{
    skus(productIds: ["{uuid}"]) {{
      skuId
      productId
      languageId
      printingId
  ...
  payload = {"query": query}
  headers = {
      "Authorization": f"Bearer {get_stored_token()}",
      "Content-Type": "application/json"
  }


  try:
      response = requests.post(MTGGRAPHQL_ENDPOINT, json=payload, headers=headers)
      response.raise_for_status()
      data = response.json()

      if "data" in data and "skus" in data["data"]:
          skus = data["data"]["skus"]
          filtered_skus = [
              sku for sku in skus
              if sku["conditionId"] in conditions and sku["printingId"] in printings
          ]
          return {"skus": filtered_skus}
      else:
          return {"error": "No SKU data found"}
...
@app.route('/process_condition_change', methods=['POST'])
def process_condition_change():
  data = request.json
  row = data.get('row')
  uuid = data.get('uuid')
  ...
  if not uuid or uuid.strip() == '':
      return jsonify({"error": f"Row {row}: No UUID available for condition change processing"}), 400

  conditions_to_fetch = [new_condition] if new_condition else ['Near Mint']

  sku_data = fetch_sku_by_uuid(uuid, conditions_to_fetch, printings_to_fetch)

  if "error" in sku_data:
      return jsonify({"error": sku_data["error"]}), 500

  if sku_data.get("skus"):
      formatted_sku = format_sku_data(sku_data)
      return jsonify({"success": True, "sku": formatted_sku})
  else:
      return jsonify({"success": False, "message": "SKU not found for criteria"})

def format_sku_data(sku_data):
  # Implement SKU data formatting logic here
  return str(sku_data)
